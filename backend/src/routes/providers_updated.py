...
def get_provider_profile(current_user):
    ...
    try:
        ...
        # Get current location (most recent online location)
        current_location = ProviderLocation.query.filter_by(
            provider_id=provider.id,
            is_online=True
        ).order_by(ProviderLocation.created_at.desc()).first()
...
# Updated location tracking with online status
...
# New endpoint to toggle online/offline status
...
# New endpoint to get online providers for customers
...
def get_online_providers():
    ...
    try:
        ...
        # Get distinct providers (latest location only)
...
def get_provider_public_profile(provider_id):
    ...
    try:
        ...
        profile_data.update({
            ...
            'is_online': current_location is not None,
            'last_seen': current_location.created_at.isoformat() if current_location else None,
            'current_location': {
